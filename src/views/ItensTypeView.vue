<template>
    <BaseLayout>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item in itens" :key="item.id" class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex px-3 py-1 text-xs font-medium rounded-lg bg-green-100 text-green-800">
                                        {{ item.categoria }}
                                    </span>
                                </td>
        ...
        <ModalItem :isOpen="isModalOpen" :itemData="selectedItem" :isEdit="isEditMode" :locais="locais"
...
function getLocalNome(localId) {
    const local = locais.value.find(l => l.id === localId);
