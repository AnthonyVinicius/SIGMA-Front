    ...
    <div class="space-y-10 px-4">
      <header class="flex flex-col sm:flex-row sm:items-center justify-between border-b pb-6">
        ...
        <button
          ...
          class="bg-[#1C5E27] hover:bg-[#174a20] text-white font-semibold py-2.5 px-5 rounded-md flex items-center gap-2 transition-all text-sm shadow-sm"
        >
      ...
      <section>
        ...
        <div v-else-if="itens.length === 0" class="flex flex-col items-center justify-center py-16">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-300 mb-3">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M9 12h6m-3-3v6m9-9a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <p class="text-gray-500 text-sm">Nenhum item encontrado neste local.</p>
        </div>
        ...
        <ul v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <li
            v-for="item in itens"
            :key="item.id"
            class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all p-5 flex flex-col justify-between"
          >
            <div class="space-y-1">
